<script>
    ...
    const catalogActions = [
        ...
        },
        {
            ...
            id: "time-series-viewer",
            icon: "clock-forward",
            label1: "Time Series",
            label2: "Catalog",
        },
        {
            name: "Summarize My Area",
            id: "sma",
    ...
    let eaTopics = getEaData("/ea/api/subtopics", topicParams)
        .then((data) => {
            ...
            dataReduced = dataReduced.map((obj) => ({
                ...obj,
                isVisible: true,
            }));
    ...
    eaTopics
        .then((result) => getEaSubtopics(result))
        .then(() => ($geography = "CONUS"))
        .then(() => countMaps());
    ...
    function handleCatalogActionClick() {
        ...
        if (nextDataCatalog != $catalog.type) {
            ...
            document
                .querySelector(`[data-panel-id=${activeDataCatalog}]`)
                .setAttribute("hidden", "");
            //TODO: do this with svelte bindings
            document.querySelector(
                `#catalog-button-${activeDataCatalog}`,
            ).style.borderBottom = "none";

            document
                .querySelector(`[data-panel-id=${nextDataCatalog}]`)
                .removeAttribute("hidden");
            //TODO: do this with svelte bindings
            document.querySelector(`#${this.id}`).style.borderBottom =
                "3px solid #162e51";
    ...
    function toggleChevron() {
        this.icon == "chevrons-right"
            ? this.setAttribute("icon", "chevrons-left")
            : this.setAttribute("icon", "chevrons-right");
    }
</script>
...
<div style="display:flex; justify-content: space-around width:100%">
    {#each catalogActions as cat, i}
        ...
        >
            ...
            <p style="line-height: 0.33em; margin: 0; padding-top:5px">
                {cat.label1}
            </p>
...
<calcite-panel data-panel-id="data-catalog" id="data-catalog">
    ...
    >
        <p class="tab-description">
            Search, filter by benefit categories, or explore EnviroAtlas map
            layers by topic below
        </p>
        <CatalogActionBar
            totalVisibleMaps={$totalVisibleMaps}
            totalMapsCount={$totalMaps}
            type={$catalog.type}
        />
        <calcite-list
            label="toc"
